# Generate CSV PSD's for 100 Healthy Controls from external healthy controls dataset
...
##################################################################################################################################################################
# Load and process NMT Scalp Normal dataset
##################################################################################################################################################################
files = os.listdir(os.path.join("Data","Healthy100"))
for file in files:
    raw = mne.io.read_raw_edf(os.path.join("Data", "Healthy100", file), infer_types = True)
    raw.load_data()
    raw.rename_channels({"FP1": "Fp1", "FP2": "Fp2", "FZ": "Fz", "PZ":"Pz", "CZ":"Cz"})
    ...
    subset["Condition"] = 0
    subset["Subject"] = "NMTHealthy_" + file
    dataset = pd.concat([dataset, subset], axis=0 ,ignore_index = True)

dataset = normalize(dataset)
dataset.to_csv("Dataset_Healthy100.csv", index = False)