...
dataset.drop(dataset[dataset.Condition == 0].index, inplace=True) # 31
healthy = pd.read_csv("Dataset_Healthy.csv") # 31
dataset = pd.concat([dataset, healthy], axis=0 ,ignore_index = True)
...
stressed = dataset.iloc[preds >= 0.5,:] # retain for review

dataset = dataset.iloc[preds < 0.5,:] # keep low stressed, below this threshold
X = dataset.iloc[:, :75]
y = dataset.iloc[:,75:76]["Condition"]
y = y.values
...
model = xgb.XGBClassifier(objective="multi:softmax", num_class=3, seed=42, n_estimators=200, max_depth=2, eta=0.1, subsample=0.7, colsample_bytree=0.3, \
...
# Test F1 Score : [0.95238095 0.90909091 1.]
# Test weighted F1 Score : 0.94

# XAI with SHAP
model = xgb.XGBClassifier(objective="multi:softmax", num_class=3, seed=42, n_estimators=200, max_depth=2, eta=0.1, subsample=0.7, colsample_bytree=0.3, \
                          eval_metric=["auc"], num_rounds=14,)
model.fit(X_train, y_train, eval_set=[(X_train, y_train), (X_test, y_test)])
...
for subject in subjects:
    ...
    y = y.values
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size = 0.30)
    model = model = xgb.XGBClassifier(objective="multi:softmax", num_class=3, seed=42, n_estimators=200, max_depth=2, eta=0.1, subsample=0.7, colsample_bytree=0.3, \
...
stress = pd.read_csv("Dataset_Stress.csv")
stress_mean = stressed.iloc[:,:75].mean(axis=0)
healthy = pd.read_csv("Dataset_Healthy.csv")
healthy_mean = healthy.iloc[:,:75].mean(axis=0)
compensated = stressed.iloc[:,:75] - (stress_mean - healthy_mean)
compensated["Condition"] = stressed["Condition"]
compensated["Subject"] = stressed["Subject"]
dataset = pd.concat([dataset, compensated], ignore_index=True)
