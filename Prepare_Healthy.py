# Generate CSV PSD's for 31 Healthy Controls from external healthy controls dataset
...
##################################################################################################################################################################
files = os.listdir(os.path.join("Data","Healthy"))
for file in files:
    raw = mne.io.read_raw_edf(os.path.join("Data", "Healthy", file), infer_types = True)
    raw.load_data()
...
dataset.to_csv("Dataset_Healthy.csv", index = False)